...
namespace ohko_gtavc
{
	public class ProcessSelectionForm : Form
	{
		...
		/// <param name="preselectName">Name of the process that should be pre-selected.</param>
		public ProcessSelectionForm(string preselectName)
			: this()
		{
		...
		private void ProcessSelectionForm_Shown(object sender, EventArgs e)
		{
			...
			// Preselect first process that matches the specified name
			if (preselectName != null)
			{
				var preselectItem = processList.Items.Cast<ListViewItem>().FirstOrDefault(
					item => GetProcessName(item).Equals(preselectName, StringComparison.OrdinalIgnoreCase));
